...
const Dashboard = () => {
  ...
  useEffect(() => {
    const savedStats = JSON.parse(localStorage.getItem("The Rose ClinicStats"));
    if (savedStats) {
      setStats(savedStats);
    }
  }, []);
  ...
  const barData = {
    ...
    datasets: [
      {
        label: "Penjualan The Rose Clinic",
  ...
  const lineData = {
    ...
    datasets: [
      {
        label: "Jumlah Pelanggan The Rose Clinic",
  ...
  return (
    <div className="p-6 space-y-8 min-h-screen bg-gradient-to-br from-indigo-50 via-indigo-100 to-indigo-200">
      <div className="mb-6">
        <h1 className="text-3xl font-extrabold text-[#181C68] drop-shadow-sm">Klinik The Rose Clinic</h1>
